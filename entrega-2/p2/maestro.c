...
void * procesaPeticion(petinfo *pet)
{
  ...
  int poslista;
  char valido=1;
  char *save_ptr; //Fase 3

  // Recibir mensaje
  leidos=read(pet->puerto, buffer, MAX_LINEA);
  if (leidos==0) return NULL; //Fase 1

  //interpretar el mensaje
  ptr = strtok_r(buffer, " \t", &save_ptr); //Fase 3
  switch (*ptr)
  {
    case 'l':
    case 'L':
	ptr = strtok_r(NULL, " \t", &save_ptr); //Fase 3
        int palillo1 = atoi(ptr);
	ptr = strtok_r(NULL, " \t", &save_ptr);  //Fase 3
        int palillo2 = atoi(ptr);

        if (palillo1 < 0 || palillo1 >= pet->tamlista || palillo2 < 0 || palillo2 >= pet->tamlista)
        {
            valido = 0;
            break;
        }
        //esperar a tener el mutex que protege al recurso solicitado
	pthread_mutex_lock(&pet->lista[palillo1]);  //Fase 3
        pthread_mutex_lock(&pet->lista[palillo2]);  //Fase 3
    ...
    case 'U':
	ptr = strtok_r(NULL, " \t", &save_ptr); //Fase 3
        int palillo3 = atoi(ptr);
	ptr = strtok_r(NULL, " \t", &save_ptr); //Fase 3
        int palillo4 = atoi(ptr);

        if (palillo3 < 0 || palillo3 >= pet->tamlista || palillo4 < 0 || palillo4 >= pet->tamlista)
        {
            valido = 0;
            break;
        }

        pthread_mutex_unlock(&pet->lista[palillo3]);  //Fase 3
        pthread_mutex_unlock(&pet->lista[palillo4]);  //Fase 3
        break;
     default:
      valido=0;
  }
