	...
	if(cl==NULL){
	    ...
	    pthread_mutex_unlock(&m);
            ...
            case 0:
                ...
                // A RELLENAR
		fprintf(fpsal, "%s %s ", q.nomdominio, q.tiporecord);
    		if (!es_MX_o_NS(q.tiporecord))
    		{
        		fprintf(fpsal, "%s ", q.clave);
    		}
		fprintf(fpsal, "%s\n", res->Resultado_u.msg);
                break;
            case 2:
...
int main(int argc, char *argv[])
{
    ...
    // A RELLENAR
    if (!valida_numero(argv[1]) || (num_clientes = atoi(argv[1])) <= 0 || (num_clientes = atoi(argv[1])) > MAXHILOSCLIENTE)
	{
    		fprintf(stderr, "Error: El número de clientes debe ser un entero positivo\n");
    		exit(2);
	}
    ip_srv = strdup(argv[2]);
    if (!valida_ip(ip_srv))
	{
    		fprintf(stderr, "Error: El parámetro IP del servidor no es valido\n");
    		exit(3);
	}
	...
	if (q == NULL)
    {
        fprintf(stderr, "Error: no se pudo asignar memoria para la estructura de datos del hilo\n");
        exit(8);
    }

    void *cliente_wrapper(void *arg) {
	...
	q->id_cliente=i;
	q->nom_fichero_consultas = strdup(argv[3]);

	if (pthread_create(&th[i], NULL,cliente_wrapper,(void *) q) != 0)
    {
        fprintf(stderr, "Error: no se pudo crear el hilo para el cliente %d\n", i);
        exit(9);
    }
    }
